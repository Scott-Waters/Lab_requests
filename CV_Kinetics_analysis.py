...
def get_peaks(file_name, Ru):
    ...
    if(Ru == 0):
        ...
        for i in range(len(Current)):
            Current_corrected.append(Current[i] - (Voltage[i] / Ru))

    #finds max/min current
    max_A = max(Current_corrected)
    min_A = min(Current_corrected)
    #finds the index of the max/min values
    max_A_ind = Current_corrected.index(max_A)
    min_A_ind = Current_corrected.index(min_A)
    #finds the voltage at the corresponding max. index
    max_V = Voltage[max_A_ind]
    min_V = Voltage[min_A_ind]
...
if (choice == "red" or choice == "ox" or choice == "both"):
    ...
    file_names = input("Please list all .dta files of the experiment separated by \', \': ")
